...
function sendToDiscordWebhook(newItem) {
    ...
    .then((response) => {
        if (response.ok) {
            console.log("Item data sent to Discord successfully.");
        } else {
            console.error("Failed to send item data to Discord.");
        }
    })
    .catch((error) => {
        console.error("Error sending item data to Discord:", error);
    });
}